...
var Node = P(function(_) {
  ...
  _.jQize = function(jQ) {
    ...
    for (var i = 0; i < jQ.length; i++) jQadd(jQ[i]);
    return jQ;
  };
...
var Controller = P(function(_) {
  ...
  _.notify = function() {
    for (var i = 0; i < notifyees.length; i++) {
...
function getInterface(v) {
  ...
  APIClasses.EditableField = P(AbstractMathQuill, function(_, super_) {
    ...
    _.click = function() { this.__controller.textarea.click(); return this; };
    ...
    _.typedText = function(text) {
      for (var i = 0; i < text.length; i++) this.__controller.typedText(text.charAt(i));
      return this;
    };
...
var saneKeyboardEvents = (function() {
  ...
  return function saneKeyboardEvents(el, handlers) {
    ...
    target.bind('keydown keypress input keyup clickout paste', function(e) { checkTextarea(e); });
    ...
    function onKeydown(e) {
      ...
      if (shouldBeSelected) checkTextareaFor(function(e) {
        if (!(e && e.type === 'clickout') && textarea[0].select) {
    ...
    function onPaste(e) {
      ...
      // when the textarea is not necessarily clicked.  We click it
      // here to ensure that the pasted text actually ends up in the
      // textarea.
      //
      // It's pretty nifty that by changing click in this handler,
...
Controller.open(function(_) {
  _.clickBlurEvents = function() {
    var ctrlr = this, root = ctrlr.root, cursor = ctrlr.cursor;
    var blurTimeout;
    ctrlr.textarea.click(function() {
      ctrlr.blurred = false;
      clearTimeout(blurTimeout);
      ctrlr.container.addClass('mq-clicked');
      if (!cursor.parent)
    ...
    function blur() { // not directly in the textarea blur handler so as to be
      ...
      ctrlr.container.removeClass('mq-clicked'); // clearing/blurring selection
...
/**
 * TODO: I wanted to move MathBlock::click and blur here, it would clean
 * up lots of stuff like, TextBlock::click is set to MathBlock::click
 * and TextBlock::blur calls MathBlock::blur, when instead they could
 * use inheritance and super_.
 *
 * Problem is, there's lots of calls to .click()/.blur() on nodes
 * outside Controller::clickBlurEvents(), such as .postOrder('blur') on
 ...
 * of them).
 *
 * I'm not even sure there aren't other troublesome calls to .click() or
...
Controller.open(function(_) {
  ...
  _.staticMathTextareaEvents = function() {
    ...
    .click(function() { ctrlr.blurred = false; }).blur(function() {
...
var MathCommand = P(MathElement, function(_, super_) {
  ...
  */
  ...
  _.html = function() {
    ...
    for (var i = 0, token = tokens[0]; token; i++, token = tokens[i]) {
...
var TextBlock = P(Node, function(_, super_) {
  ...
  _.createLeftOf = function(cursor) {
    ...
    if (textBlock.replacedText)
      for (var i = 0; i < textBlock.replacedText.length; i++)
  ...
  _.click = MathBlock.prototype.click;
});
...
var Letter = P(Variable, function(_, super_) {
    ...
    if (maxLength > 0) {
      ...
      while (str.length) {
        if (autoCmds.hasOwnProperty(str)) {
          for (var i = 2, l = cursor[L]; i < str.length; i++, l = l[L]);
  ...
  _.autoUnItalicize = function(opts) {
    ...
    outer: for (var i = 0, first = l[R] || this.parent.ends[L]; i < str.length; i++, first = first[R]) {
...
var mostOps = ('ln nCr nPr cos sin tan acos asin atan').split(' ');
for (var i = 0; i < mostOps.length; i++) {
...
var OperatorName = P(P(MathCommand, DelimsMixin), function(_, super_) {
  _.init = function(fn) { 
    this.ctrlSeq = fn; 
    this.htmlTemplate = `<var class="mq-operator-name">${fn}</var><span class="mq-non-leaf"><span class="mq-scaled mq-paren">(</span><span class="mq-non-leaf"><span>&0</span></span><span class="mq-scaled mq-paren">)</span></span>`;
    this.textTemplate = [`${fn}(`, ')'];
...
LatexCmds['\u00d7'] = LatexCmds.times = CharCmds['*'] = LatexCmds.sdot = LatexCmds.cdot =
...
var SupSub = P(MathCommand, function(_, super_) {
  ...
  _.addBlock = function(block) {
    ...
    for (var i = 0; i < 2; i++) (function(cmd, supsub, oppositeSupsub, updown) {
...
LatexCmds.log = P(SupSub, function(_, super_) {
  _.supsub = 'sub';
  _.htmlTemplate = '<var class="mq-operator-name">log</var><span class="mq-supsub mq-non-leaf"><span class="mq-sub">&0</span><span class="mq-un">​</span></span><span class="mq-non-leaf"><span class="mq-scaled mq-paren">(</span><span class="mq-non-leaf"><span>&1</span></span><span class="mq-scaled mq-paren">)</span></span>';
  _.textTemplate = ['log(', ',', ')'];
  _.finalizeTree = function() {
    this.downInto = this.sub = this.ends[L];
    this.sub.upOutOf = insLeftOfMeUnlessAtEnd;
